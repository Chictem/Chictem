            ...
            <div class="col-md-12">
                ...
                <div class="panel panel-bordered">
                    ...
                    <!-- CSRF TOKEN -->
                        ...
                        <!-- If we are editing -->
                            @if(isset($dataTypeContent->id))
                                @php $dataTypeRows = $dataType->editRows; @endphp
                            @else
                                @php $dataTypeRows = $dataType->addRows; @endphp
                            @endif
                                ...
                                <div class="form-group">
                                    ...
                                    @elseif($row->type == "select_dropdown")
                                        ...
                                        @if(isset($options->relationship))
                                            ...
                                            @if( method_exists( $dataType->model_name, camel_case(str_replace('_id', '', $row->field))))
                                                @php $selected_value = (isset($dataTypeContent->{$row->field}) && ! is_null(old($row->field, $dataTypeContent->{$row->field}))) ? old($row->field, $dataTypeContent->{$row->field}) : old($row->field); @endphp
                                                <select class="form-control select2" name="{{ $row->field }}">
                                                    @php $relationshipClass = get_class(app($dataType->model_name)->{camel_case(str_replace('_id', '', $row->field))}()->getRelated()); @endphp
                                                    @php $relationshipOptions = $relationshipClass::all(); @endphp
                                    ...
                                    @elseif($row->type == "select_multiple")
                                        ...
                                        {{-- If this is a relationship and the method does not exist, show a warning message --}}
                                        @if(isset($options->relationship) && !method_exists( $dataType->model_name, camel_case(str_replace('_id', '', $row->field))) )
                                            <p class="label label-warning"><i class="voyager-warning"></i> Make sure
                                                to setup the appropriate relationship in
                                                the {{ camel_case(str_replace('_id', '', $row->field)) . '()' }} method
                                                of
                                                the {{ $dataType->model_name }} class.</p>
                                        @endif
                                        ...
                                        @if(isset($options->relationship))
                                            <!-- Check that the method relationship exists -->
                                                @if( method_exists( $dataType->model_name, camel_case(str_replace('_id', '', $row->field))) )
                                                    @php $selected_values = isset($dataTypeContent) ? $dataTypeContent->{$row->field}()->getRelated()->pluck($options->relationship->key)->all() : array(); @endphp
                                                    @php $relationshipClass = get_class(app($dataType->model_name)->{camel_case(str_replace('_id', '', $row->field))}()->getRelated()); @endphp
                                                    @php $relationshipOptions = $relationshipClass::all(); @endphp
                                                    @foreach($relationshipOptions as $relationshipOption)
                                    ...
                                    @elseif($row->type == "radio_btn")
                                        ...
                                        <ul class="radio">
                                            @if(isset($options->options))
                                                @foreach($options->options as $key => $option)
                                                    <li>
                                                        <input type="radio" id="option-{{ $key }}-{{ $row->field }}"
                                                               name="{{ $row->field }}"
                                                               value="{{ $key }}" @if($default == $key && !$selected_value){{ 'checked' }}@endif @if($selected_value == $key){{ 'checked' }}@endif>
                                                        <label for="option-{{ $key }}-{{ $row->field }}">{{ $option }}</label>
                                                        <div class="check">{{ $selected_value }}</div>
                                                    </li>
                                                @endforeach
                                            @endif
                                        </ul>
                                    ...
                                    @elseif($row->type == "checkbox")
                                        ...
                                        <br>
                                        @php $options = json_decode($row->details); @endphp
                                        @php $checked = (isset($dataTypeContent->{$row->field}) && old($row->field, $dataTypeContent->{$row->field}) == 1) ? true : old($row->field); @endphp
