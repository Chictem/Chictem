...
@section('page_header')
    <h1 class="page-title">
        <i class="voyager-data"></i> {{ trans('admin.database.name') }}
        <a href="{{ route('voyager.database.create') }}" class="btn btn-success"><i class="voyager-plus"></i>
            {{ trans('admin.database.button.add_table') }}</a>
    </h1>
@stop
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <table class="table table-striped database-tables">
                    <thead>
                    <tr>
                        <th>{{ trans('admin.database.attribute.name') }}</th>
                        <th>{{ trans('admin.database.attribute.bread') }}</th>
                        <th style="text-align:right">{{ trans('common.button.action') }}</th>
                    </tr>
                    </thead>
                                ...
                                <div class="bread_actions">
                                    @if($active)
                                        <a class="btn-sm btn-default edit"
                                           href="{{ route('voyager.database.edit_bread', $activeDataType->id) }}"> {{ trans('admin.database.button.edit') }}
                                            </a>
                                        <div class="btn-sm btn-danger delete" style="display:inline"
                                             data-id="{{ $activeDataType->id }}" data-name="{{ $table }}"> {{ trans('admin.database.button.delete') }}
                                        </div>
                                    @else
                                        <form action="{{ route('voyager.database.create_bread') }}" method="POST">
                                            ...
                                            <button type="submit" class="btn-sm btn-default"><i
                                                        class="voyager-plus"></i> {{ trans('admin.database.button.add') }}
                                            </button>
                                        </form>
                                    @endif
                                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form action="{{ route('voyager.database.delete_bread', ['id' => null]) }}" id="delete_builder_form"
                          method="POST">
                        {{ method_field('DELETE') }}
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="voyager-trash"></i>
                        {!! trans('common.alert.delete', ['name' => '<span id="delete_table_name"></span>']) !!}
                    </h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('voyager.database.destroy', ['database' => '__database']) }}"
                          id="delete_table_form" method="POST">
                        {{ method_field('DELETE') }}
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        ...
                        <button type="button" class="btn btn-outline pull-right" style="margin-right:10px;"
                                data-dismiss="modal"> {{ trans('common.button.cancel') }}
                        </button>
                    </form>
