...
class VoyagerServiceProvider extends ServiceProvider
{
		...
		if (config('voyager.user.add_default_role_on_register')) {
			...
			$app_user::created(function ($user) {
				if (is_null($user->role_id)) {
					User::findOrFail($user->id)->setRole(config('voyager.user.default_role'))->save();
				}
			});
		}

		$this->loadViewsFrom(__DIR__ . '/../../resources/views/admin', 'voyager');
	...
	protected function addStorageSymlinkAlert()
	{
		...
		if ($routeName == 'voyager.dashboard' && request()->has('fix-missing-storage-symlink') && ! file_exists(public_path('storage'))) {
			...
			$alert = (new Alert('missing-storage-symlink', 'warning'))->title('Missing storage symlink')->text('We could not find a storage symlink. This could cause problems with loading media files from the browser.')->button('Fix it', '?fix-missing-storage-symlink=1');
	...
	protected function fixMissingStorageSymlink()
	{
		...
		if (file_exists(public_path('storage'))) {
			$alert = (new Alert('fixed-missing-storage-symlink', 'success'))->title('Missing storage symlink created')->text('We just created the missing symlink for you.');
		} else {
			$alert = (new Alert('failed-fixing-missing-storage-symlink', 'danger'))->title('Could not create missing storage symlink')->text('We failed to generate the missing symlink for your application. It seems like your hosting provider does not support it.');
		}
	...
	protected function registerAlertComponents()
	{
		...
		foreach ($components as $component) {
			$class = 'App\\Components\\'.ucfirst(camel_case($component)).'Component';
