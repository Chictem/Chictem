...
if (! function_exists('isBreadSlugAutoGenerator')) {
	...
	function isBreadSlugAutoGenerator($options)
	{
		if (isset($options->slugify)) {
			return ' data-slug-origin=' . $options->slugify->origin . ((isset($options->slugify->forceUpdate)) ? ' data-slug-forceupdate=true' : '');
		}
	}
}
...
if (! function_exists('isFieldSlugAutoGenerator')) {
	...
	function isFieldSlugAutoGenerator($dataTypeContent, $field)
	{
		...
		$_tmp = (isset($dataTypeContent->id)) ? $dataType->editRows : $dataType->addRows;

		$_tmp = $_tmp->filter(function ($val) use ($field) {
